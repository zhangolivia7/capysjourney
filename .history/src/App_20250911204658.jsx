...
function WaitlistForm() {
  ...
  async function handleSubmit(e) {
    e.preventDefault()
    console.log('Form submitted, gasUrl:', gasUrl)
    console.log('Environment variables:', import.meta.env)
    
    if (!gasUrl || gasUrl.includes('YOUR_DEPLOYMENT_ID') || gasUrl.includes('undefined')) {
      // For now, just simulate success since Google Apps Script isn't set up yet
      setStatus('loading')
      setTimeout(() => {
        setStatus('success')
        setName('')
        setEmail('')
        console.log('Form data (simulated):', { name, email, source: 'capysjourney-landing' })
        alert('Form submitted successfully! (This is a demo - data logged to console)')
      }, 1000)
      return
    }
    ...
    setStatus('loading')
    try {
      console.log('Sending request to:', gasUrl)
      ...
      try {
        ...
        console.log('Test request failed:', testErr.message)
        // Continue anyway, sometimes GET fails but POST works
      }
      ...
      // If it's a CORS or network error, fall back to simulation
      if (err.message.includes('Failed to fetch') || err.message.includes('CORS')) {
        console.log('Falling back to simulation due to CORS/network error')
        setStatus('loading')
        setTimeout(() => {
          setStatus('success')
          setName('')
          setEmail('')
          console.log('Form data (simulated due to CORS):', { name, email, source: 'capysjourney-landing' })
          alert('Form submitted successfully! (Simulated due to CORS issue - data logged to console)')
        }, 1000)
        return
      }
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {status === 'error' && (
            <div className="text-red-700 text-center">
              ...
              <p className="text-xs mt-1">Check the browser console for details.</p>
            </div>
          )}
        </form>
      </div>
      ...
      <div className="mx-auto max-w-[1440px]">
        ...
        <section className="relative w-full flex flex-col" style={{ background: brandCream }}>
          <div className="flex-1 flex flex-col justify-between" style={{ background: brandCream }}>
            ...
            <div className="px-6 flex justify-center mb-16">
        ...
        <section className="px-6 py-16 md:py-24" style={{ background: brandCream }}>
          <div className="mx-auto max-w-6xl grid md:grid-cols-2 gap-10 items-center">
            ...
            <p className="text-2xl md:text-3xl text-right" style={{ color: brandBrown }}>Over __ fun, expert-certified level to play through.</p>
          </div>
        </section>
