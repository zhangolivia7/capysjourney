  ...
  async function handleSubmit(e) {
    ...
    if (!gasUrl || gasUrl.includes('YOUR_DEPLOYMENT_ID') || gasUrl.includes('undefined')) {
      ...
      setTimeout(() => {
        ...
        setName('');
        setEmail('');
        console.log('Simulated:', { name, email, source: 'capysjourney-landing' });
    ...
    try {
      const body = new URLSearchParams({
        name,
        email,
        source: 'capysjourney-landing'
      });
  
      // IMPORTANT: urlencoded avoids preflight; keep headers minimal
      await fetch(gasUrl, {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8' },
        body
        // DO NOT set mode: 'no-cors' (you won’t need it, and it hides errors)
      });
